      ...
      model.Entry.findById(entryId,
        function(err, doc) {
          ...
          if(doc) {
            ...
            function saveFile() {
              var originalName = req.headers['x-file-name'],
              ...
              for(var i=0;i<req.chunks.length;i++) {
                data += req.chunks[i].toString('binary');
              }
              ...
              fs.writeFile(fileName, data, 'binary',
                function(err) {
                  ...
                  doc.photo = path.join('/uploads', newName);
                  doc.hashtag = "cow" + ((data.gender === 'male') ? 'boy' : 'girl') + count;
                  doc.save();
