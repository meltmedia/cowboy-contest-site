  ...
  var possibleVotes = _.pluck(data.entities.user_mentions, 'screen_name'),
        ...
        model.Vote.findOne({voterId: record.voterId}, function(err, doc) {
          ...
          if (doc) {
            ...
            model.Entry.update({twitter: doc.votedFor}, { $inc: { votes: -1 }}, function(err, numAffected) {
              if (err) throw err;
              if (numAffected > 0) {
                app.log.debug('VOTE: ' + doc.votedFor + ' -1');
              } else {
                app.log.warn('VOTE: could not decrement entry ' + doc.votedFor);
              }
            });
          }
          ...
          model.Entry.update({twitter: record.votedFor}, { $inc: { votes: 1 }}, function(err, numAffected) {
            ...
            if (numAffected > 0) {
              ...
              model.Vote.update({voterId: record.voterId}, record, {upsert: true}, function(err, numAffected) {
                if (err) throw err;
                if (numAffected > 0) {
                  app.log.debug('VOTE: ' + record.votedFor + ' +1');
                } else {
                  app.log.warn('VOTE: could not update ' + record.voterTwitter + ' vote for ' + record.votedFor);
                }
              });
            } else {
              app.log.warn('VOTE: could not increment entry ' + record.votedFor);
            }
          });
      ...
      validateNextContestant = function() {
        if (possibleVotes.length > 0) {
          ...
          app.log.debug('No valid contestants in tweet' + data.id_str);
        }
      };
...
var streamMethods = {
  start: function() {
    twitter.stream('statuses/filter', {track: '#'+contestHashtag}, function(stream) {
      app.log.debug('STREAM: starting');
      streamMethods.reset();
      streamMethods.destroyCountdown(stream);
      stream
        .on('data', validateVote)
        .on('error', function(error) {
          app.log.debug('STREAM: error called', error);
          streamMethods.onError(error);
        })
        .on('end', function(response) {
          app.log.debug('STREAM: end called, Override', superDestroyOverride);
          if (!superDestroyOverride) {
            streamMethods.linear();
          }
        })
        .on('destroy', function(response) {
          app.log.debug('STREAM: destroy called, Override', superDestroyOverride);
          if (!superDestroyOverride) {
            streamMethods.linear();
          }
        });
  ...
  startCountdown: function() {
    ...
    if (startStreamIn !== null) {
      app.log.debug('STREAM: Start in ' + startStreamIn + ' seconds');
  ...
  destroyCountdown: function(stream) {
    ...
    if (streamDestoryIn !== null) {
      app.log.debug('STREAM: destroy in ' + streamDestoryIn + ' seconds');
      destroyTimeout = setTimeout(function() {
        app.log.debug('STREAM: times up, destroying');
  ...
  linear: function() {
    app.log.debug('STREAM: applying linear wait', linearCurrentWait);
  ...
  exponential: function() {
    app.log.debug('STREAM: applying exponential wait', exponentialCurrentWait);
    clearTimeout(waitTimeout);
