...
var twitter = require('ntwitter'),
   ...
   streamStart = new Date(Date.parse("Sun Apr 1 15:30:00 -0700 2012")),
   streamEnd = new Date(Date.parse("Sun Apr 1 16:30:00 -0700 2012")),
   destroyTimeout = null,
   startTimeout = null,
   waitTimeout = null,
          ...
          if (doc) {
            ...
            model.Entry.update({twitter: doc.votedFor}, { $inc: { votes: -1 }}, function(err, numAffected) {
              ...
              if (numAffected > 0) {
                ...
                app.log.warn('Could not decrement entry ' + doc.votedFor);
              }
            });
          }
            ...
            if (numAffected > 0) {
              ...
              model.Vote.update({voterId: record.voterId}, record, {upsert: true}, function(err, numAffected) {
                ...
                if (numAffected > 0) {
                  ...
                  app.log.warn('Could not update ' + record.voterTwitter + ' vote for ' + record.votedFor);
                }
              });
            } else {
              app.log.warn('Could not increment entry ' + record.votedFor);
            }
          });
...
var streamMethods = {
  start: function() {
    twitter.stream('statuses/filter', {track: '#'+contestHashtag}, function(stream) {
      app.log.debug('Starting Twitter stream');
      streamMethods.reset();
  ...
  startCountdown: function() {
    ...
    if (startStreamIn !== null) {
      app.log.debug('Start Twitter stream in ' + startStreamIn + ' seconds');
  ...
  destroyCountdown: function(stream) {
    ...
    if (streamDestoryIn !== null) {
      app.log.debug('Destroy Twitter stream in ' + streamDestoryIn + ' seconds');
      destroyTimeout = setTimeout(function() {
        app.log.debug('Destroying Twitter stream');
  ...
  linear: function() {
    app.log.debug('linear wait', linearCurrentWait);
  ...
  exponential: function() {
    app.log.debug('exponential wait', exponentialCurrentWait);
    clearTimeout(waitTimeout);
